    ...
    difference() {
        union() {
            ...
            intersection() {
                ...
                cylinder(bottleneck_length + bottleneck_radius, bottleneck_radius, bottleneck_radius);
            }
                ...
                for (phi = [360/(support_count*2):360/support_count:360-epsilon]) {
                    ...
                    linear_extrude(support_thickness)
                        difference() {
                            ...
                            gate([support_diameter / 2 - bottleneck_radius * 2 + pipe_radius, support_height/1.5]);
                        }
                }
                cylinder(support_height, support_diameter/2, support_diameter/2);
            }

            fwd(feed_radius + wall_thickness)
                rotate([90,0,90])
                    linear_extrude(feed_length)
                        gate([feed_width + wall_thickness * 2, feed_width + wall_thickness * 2]);

            cylinder(wall_thickness, support_diameter/2, support_diameter/2);

            difference() {
                cylinder(support_height / 4, support_diameter/2, support_diameter/2);
                cylinder(support_height / 4 + epsilon, support_diameter/2 - wall_thickness, support_diameter/2 - wall_thickness);
            }
        }
        up(wall_thickness)
            cylinder(bottle_height + support_height, pipe_radius - wall_thickness, pipe_radius - wall_thickness);

        up(wall_thickness)
            fwd(feed_radius)
                rotate([90,0,90])
                    linear_extrude(feed_length + epsilon)
                        gate([feed_width, feed_width]);
    }
    difference() {
        intersection() {
        ...
        translate([0, feed_radius, feed_radius + wall_thickness])
            xrot(90)
